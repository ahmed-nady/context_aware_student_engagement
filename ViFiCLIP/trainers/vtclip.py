...
from trainers.mit import MultiframeIntegrationTransformer
...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'Vt_CLIP',
...
class VTCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, logger):
        super().__init__()
        self.prompt_learner = VLPromptLearner(cfg, classnames, clip_model, logger)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.image_encoder = clip_model.visual
        self.mit = MultiframeIntegrationTransformer(T=32, embed_dim=512, layers=1,)
    ...
    def forward(self, image,few_shots=False):
        ...
        image_cls_token, image_features = self.image_encoder(image.type(self.dtype))
        # Now again attach the batch dimensions
        image_features = image_features.view(b, t, -1)  # [B, T, 512]
        cls_features = image_features
...
def returnCLIP(config, logger=None,
    ...
    model = VTCLIP(config, class_names, clip_model, logger)
