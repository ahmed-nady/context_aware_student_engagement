...
from ViFiCLIP.clip import clip
from ViFiCLIP.clip.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class TemporalAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        attention = self.sigmoid(x.view(bs,t,1))
...
class ViFiCLIP(nn.Module):
    ...
    def forward(self, image,few_shots=False):
        ...
        #image_features = self.temporal_attention(image_features)
        ...
        #mean_image_features = image_features.mean(dim=1, keepdim=False)  # image features are now ready
        # # Now take the max along the temporal direction
        ##image_features,_ = torch.max(image_features,dim=1, keepdim=False)  # image features are now ready
        # image_features = torch.cat((mean_image_features,max_image_features),dim=-1)
        # image_features = self.project_fc(image_features)
        few_shots_image_features = image_features
        ...
        #=====add cosine similarity between vids as well.
        #vids_logits=logit_scale * image_features @ image_features.t()
        if few_shots:
            return logits,few_shots_image_features
