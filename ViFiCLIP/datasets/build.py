...
import pandas as pd
import json
with open('/media/hd2/Nady/ActionRecognition/AAActionRecognition/VLM/ViFi-CLIP/motion_guided_sampling/img_diff_studentActions_train.json', 'r') as f:
    vid_train = json.load(f)

with open('/media/hd2/Nady/ActionRecognition/AAActionRecognition/VLM/ViFi-CLIP/motion_guided_sampling/img_diff_studentActions_val.json', 'r') as f:
    vid_val = json.load(f)
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def prepare_train_frames(self, idx):
        ...
        video_name = results['filename'].split("/")[-1][:-4]
        ##results['img_diff'] = vid_train[video_name]
    ...
    def prepare_test_frames(self, idx):
        ...
        results['start_index'] = self.start_index
        video_name = results['filename'].split("/")[-1][:-4]
        ##results['img_diff'] = vid_val[video_name]
        # prepare tensor in getitem
        # If HVU, type(results['label']) is dict
        if self.multi_class and isinstance(results['label'], list):
            onehot = torch.zeros(self.num_classes)
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                video_infos.append(
                    dict(filename=filename, label=label, tar=self.use_tar_format))

                # if self.train_flag:
                #     video_infos.append(dict(filename=filename, label=label, tar=self.use_tar_format,img_diff=vid_train[filename] ))
                # else:
                #     video_infos.append(dict(filename=filename, label=label, tar=self.use_tar_format,img_diff=vid_val[filename] ))
...
def build_dataloader(logger, config):
    ...
    train_pipeline = [
        ...
        dict(type='UniformSampleFrames', clip_len=config.DATA.NUM_FRAMES, num_clips=1),
        #dict(type='MGSampleFrames', clip_len=config.DATA.NUM_FRAMES, num_clips=1),
    ...
    val_pipeline = [
        ...
        dict(type='UniformSampleFrames', clip_len=config.DATA.NUM_FRAMES, num_clips=1, test_mode=True),
        #dict(type='MGSampleFrames', clip_len=config.DATA.NUM_FRAMES, num_clips=1,test_mode=True),
    ...
    val_data = VideoDataset(ann_file=config.DATA.VAL_FILE, data_prefix=config.DATA.ROOT, labels_file=config.DATA.LABEL_LIST, pipeline=val_pipeline,test_mode=True)
